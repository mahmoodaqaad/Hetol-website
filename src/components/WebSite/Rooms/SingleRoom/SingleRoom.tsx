...
const SingleRoom = ({ room, booking = true }: { room: RoomWithReltionAll, booking: boolean }) => {
    ...
    return (
        <div className='bg-gray-200 dark:bg-gray-800 dark:text-white shadow-xl relative p-3 rounded-lg    overflow-hidden transition-all hover:-translate-y-3'>
            <Link href={`/rooms/${room.id}`} className='relative '>
                    ...
                    src={room.images[0].imageUrl} alt='' className='object-cover mx-auto h-[200px]' />
                {
                    booking &&
                    room.status == "booked" &&
                    <div className='absolute top-0 left-0 px-12    text-center py-1 text-white  bg-blue-400 -rotate-45 -translate-x-[3.4rem] -translate-y-[10.5rem] text-xl tracking-[2px] '>
                        Booked
                    </div>
                }
            </Link>
