...
export const GetRoomsRequest = async (pageNumber: string | number, search: string, sort: string, order: string, filter: string) => {
    // const token = (await cookies()).get("jwt")?.value
    const response = await getFetchAll("booking-requests", pageNumber, search, sort, order, filter)
